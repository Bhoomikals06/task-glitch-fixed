...
let initialLoadPromise: Promise<Task[]> | null = null;
...
export function useTasks(): UseTasksState {
  ...
  function normalizeTasks(input: any[]): Task[] {
    ...
    return (Array.isArray(input) ? input : []).map((t, idx) => {
      ...
      const title = (typeof t.title === 'string' && t.title.trim()) ? t.title : `Untitled ${idx + 1}`;
      return { id, title, revenue, timeTaken, priority: t.priority, status: t.status, notes: t.notes, createdAt: created.toISOString(), completedAt: completed } as Task;
    });
  }
