...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  const numericROI = tasks.filter(t => typeof t.roi === 'number' && Number.isFinite(t.roi as number));
  const roiBuckets = [
    { label: '<200', count: numericROI.filter(t => (t.roi as number) < 200).length },
    { label: '200-500', count: numericROI.filter(t => (t.roi as number) >= 200 && (t.roi as number) <= 500).length },
    { label: '>500', count: numericROI.filter(t => (t.roi as number) > 500).length },
